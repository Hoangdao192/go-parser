...
func isIgnore(file string, baseDir string, ignores *[]string) bool {
	file, _ = filepath.Abs(file)

	for _, ignore := range *ignores {
		ignore = filepath.Join(baseDir, ignore)
		ignore, _ = filepath.Abs(ignore)
		if file == ignore {
			return true
		}

		relPath, _ := filepath.Rel(ignore, file)
...
func Parse(sourceDir string, destDir string, ignores []string) {
	...
	err := filepath.Walk(sourceDir, func(file string, info fs.FileInfo, err error) error {
		if err != nil {
			return err
		}
		if isIgnore(file, sourceDir, &ignores) {
			return nil
		}

		if isGoFile(info) {
			os.MkdirAll(destDir+"/"+path.Dir(file)[len(sourceDir):], os.ModePerm)

			fileContent, readErr := util.ReadFile(file)
			if readErr == nil {
				parsedFile, parseErr := parser.ParseFile(token.NewFileSet(), info.Name(),
					fileContent, parser.AllErrors)
				if parseErr == nil {
					jsonData, jsonErr := json.Marshal(parse(file, parsedFile))
					if jsonErr == nil {
						saveFilePath := destDir + "/" + file[len(sourceDir):] + ".json"
			...
			saveFilePath := destDir + file[len(sourceDir):]
